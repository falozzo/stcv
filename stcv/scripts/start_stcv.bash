...
then
    ...
    tensorboard --logdir=${STCV_TENSORBOARD_1_DATA_DIR} --port ${STCV_TENSORBOARD_1_PORT} --reload_multifile True > /dev/null 2>&1 &
fi

if [ ${STCV_TENSORBOARD_2_ENABLE} = "True" ]
then
    sudo -u ec2-user mkdir -p ${STCV_TENSORBOARD_2_DATA_DIR}
    tensorboard --logdir=${STCV_TENSORBOARD_2_DATA_DIR} --port ${STCV_TENSORBOARD_2_PORT} --reload_multifile True > /dev/null 2>&1 &
fi
